...
@pytest.mark.asyncio
async def test_complete_paid_appointment_successful(
    make_user, make_scheduled_appointment, make_payment, write_uow, read_uow
):
    user = make_user()
...
@pytest.mark.asyncio
async def test_complete_paid_more(
    make_user, make_scheduled_appointment, make_payment, write_uow, read_uow
):
    user = make_user()
    write_uow.users.create(user)
...
@pytest.mark.asyncio
async def test_complete_paid_appointment_with_referral_successful(
    make_user,
    make_vip_client,
    ...
    assert len(transactional_bus.events) == 1
    event = transactional_bus.events[0]
...
@pytest.mark.asyncio
async def test_complete_paid_appointment_with_self_referral_successful(
    make_user,
    make_vip_client,
    make_scheduled_appointment,
    make_payment,
...
@pytest.mark.asyncio
async def test_complete_paid_appointment_create_log(
    make_user, make_scheduled_appointment, make_payment, write_uow, read_uow
):
    user = make_user()
    write_uow.users.create(user)
...
@pytest.mark.asyncio
async def test_appointment_not_fully_paid(
    make_user, make_scheduled_appointment, make_payment, write_uow, read_uow
):
    user = make_user()
    write_uow.users.create(user)
...
@pytest.mark.asyncio
async def test_appointment_was_not_in_correct_status(
    make_user, make_quoted_appointment, make_payment, write_uow, read_uow
):
    user = make_user()
    write_uow.users.create(user)
...
@pytest.mark.asyncio
async def test_appointment_not_persisted_paid(
    make_user, make_scheduled_appointment, make_payment, write_uow, read_uow
):
    user = make_user()
    write_uow.users.create(user)
