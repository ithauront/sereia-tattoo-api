...
async def test_send_user_activation_handler_sends_email():
    user_id = uuid4()
    event = ActivationEmailRequested(user_id=user_id, email="jhon@doe.com", activation_token_version=1)

    email_service = FakeEmailService()
    token_service = FakeVersionedTokenService()

    handler = SendUserActivationHandler(email_service=email_service, token_service=token_service)
