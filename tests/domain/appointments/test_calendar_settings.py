...
def test_is_inside_wrking_period_true(make_working_period, make_user):
    now = datetime.now(timezone.utc)
    future = now + timedelta(days=30)
    booking_in_future_date = future.date()
    ...
    is_inside = calendar.is_inside_working_period(start=time(9, 00), end=time(10, 00))

    assert is_inside is True


def test_is_inside_wrking_period_false(make_working_period, make_user):
    now = datetime.now(timezone.utc)
    future = now + timedelta(days=30)
    booking_in_future_date = future.date()
    ...
    is_inside = calendar.is_inside_working_period(start=time(11, 00), end=time(13, 00))
