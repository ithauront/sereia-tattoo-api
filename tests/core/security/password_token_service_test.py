...
def test_password_token_contains_user_id_and_version():
...
def test_activation_token_is_rejected_by_reset_password_service():
    jwt_service = JWTService(
        secret_key=SecretStr("test-secret"),
        algorithm="HS256",
    )
    ...
    token = activation_service.create("user-id-123", version=1)

    with pytest.raises(TokenError, match="wrong_token_type"):
        reset_password_service.verify(token)
